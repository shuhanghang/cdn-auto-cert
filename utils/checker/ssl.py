...
class SSLChecker:
    ...
    @timeout_decorator.timeout(30, timeout_exception=ShowResultTimeout, use_signals=False)
    @retry(ShowResultNull, tries=2, delay=2, backoff=2)
...
def ssl_check(domain):
    try:
        args = {'hosts': [domain]}
        check_res = SSLChecker.show_result(SSLChecker.get_args(json_args=args))
        if check_res:
            logger.info(f"ssl_checker: \"domain: {domain}, msg: 加速站点健康检查通过\"")
        return (json.loads(check_res), domain)
    except Exception as e:
        logger.warning(f"ssl_checker: \"domain: {domain}, msg: {e}\"")
        return (False, domain)
