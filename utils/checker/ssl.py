...
class ShowResultTimeout(Exception):
    """
    """
    def __str__(self):
        return "站点健康检查超时"
...
class SSLChecker:
    ...
    @timeout_decorator.timeout(3, timeout_exception=ShowResultTimeout, use_signals=False)
    @retry(ShowResultNull, tries=2, delay=1, backoff=2)
...
def check(checked_domain: CheckedCnameModel):
    domain = checked_domain.checkDomain
    try:
        args = {'hosts': [domain]}
        ...
        if check_res and g:
            ...
            if query_ssl := db.session.query(SSlOnline).filter(SSlOnline.domain == domain, SSlOnline.cdn_type == checked_domain.type).first():
                ssl = query_ssl
            ssl.domain = domain
            ssl.issuer = check_res[domain].get('issuer_o')
            ssl.cdn_type = checked_domain.type
            ssl.validity_days = check_res[domain].get('validity_days')
            ssl.days_left = check_res[domain].get('days_left')
            ssl.tcp_port = check_res[domain].get('tcp_port')
            ssl.cert_exp = check_res[domain].get('cert_exp')
            ssl.create_date = datetime.now()
            db.session.add(ssl)
            db.session.commit()
        logger.info(f"ssl_checker: 加速域名为 {domain}, 检查完成")
        return check_res, domain
    except Exception as e:
        logger.warning(f"ssl_checker: 加速域名为 {domain}, {e}")
        return False, domain
